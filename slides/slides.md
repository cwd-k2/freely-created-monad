...
- 「計算の残り」が **関数として明示化** される — reification の第一段階
- ただし関数は **中身が見えない** → DSL のインタプリタは命令を判別できない
...
> `forall b.` はデータコンストラクタ内では**存在量化**（∃b）— 構築側は `b` を選べるが、受取側には不透明
...
> 「プログラムは **個々の命令の列**（ないし木構造）である」
> → `(a -> b)` の「何かをする」= 命令を一つ実行すること
> → **命令データ + 結果を受け取る継続** に分解できる
...
> このパターンが可能なのは、プログラムが Free で**データとして具象化 (reify)** されているから
