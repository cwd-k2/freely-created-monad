...
前章で、CPS と Defunctionalization によって計算を具象化する方法を見ました。しかし継続のチェーン `ContChain` には不透明な関数 `(a -> b)` が残っていました。この章ではその関数の構造を明らかにし、自由モナドを導出します。そして、自由モナドのインタプリタが持つ**ステップ実行**——「先頭を覗き、命令を処理し、継続に値を渡す」——というパターンを認識します。これははじめに で示した軸2の核心であり、次章以降の限定継続や Generator への橋渡しの起点です。
...
これはまさにモナドの `>>=`（bind）の型であり、モナド構造が必要な理由です。

```
m >>= f
  m: 値 a を生み出す計算
  f: a を受け取って次の計算を決める関数
```
...
前章で `bindCont`——「計算の結果を受け取って次の計算に渡す」合成操作——を「本資料全体を貫く中心的なアイデア」と呼びました。`runTalkIO` のステップ実行はまさにその具象化です。命令を処理して得た値を継続（`next`）に渡し、次のステップへ進む——`bindCont` の構造が、データのパターンマッチとして目に見える形で動いています。
