...
インタプリタが命令を識別して処理を分岐するには、「このプログラムは今どんな命令を実行しようとしているのか」が外から見える必要があります。しかし通常のプログラムでは、制御フロー（「次に何をするか」）は暗黙的で、外から覗くことができません。
...
しかし、CPS では継続は**関数**です。関数は呼ぶことしかできず、「この計算は何をしようとしているのか」を外から見ることはできません。前章で掲げた「実行方法を後から差し替えられる」DSL を実現するには、インタプリタが命令の種類を識別して処理を分岐する必要があります。具象化を完成させるには、継続を関数のまま持つのではなく、**データ**にする必要があります。
