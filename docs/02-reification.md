...
前章で掲げたゴール——プログラムの定義と実行を分離し、実行方法を後から差し替えられる DSL——を実現するには、インタプリタが「このプログラムは今どんな命令を実行しようとしているのか」を識別できる必要があります。しかし通常のプログラムでは、制御フロー（「次に何をするか」）は暗黙的で、外から覗くことができません。

この章では、制御フローを**データとして見える形にする**——すなわち計算を**具象化**する——ための2つの技法を導入します。「残りの計算」を値として明示化する **CPS** と、関数をデータに変換する **Defunctionalization** です。
...
CPS に変換すると、プログラムの「残りの計算」が**関数として明示化**されます。

これにより：

- **計算の中断と再開** が表現できる（継続を保存しておけばよい）
- **計算の合成** が体系的にできる（`bindCont`）
- **制御フロー** をプログラマが自由に操作できる

これは計算の**具象化の第一段階**です。直接スタイルでは暗黙だった「残りの計算」が、CPS では継続という値として明示化されました。

しかし、CPS では継続は**関数**です。関数は呼ぶことしかできず、「この計算は何をしようとしているのか」を外から見ることはできません。ch01 で掲げた「実行方法を後から差し替えられる」DSL を実現するには、インタプリタが命令の種類を識別して処理を分岐する必要があります。具象化を完成させるには、継続を関数のまま持つのではなく、**データ**にする必要があります。
