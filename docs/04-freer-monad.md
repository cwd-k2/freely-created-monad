...
`Coyoneda f a` は「`f x` の値と、結果を `x` から `a` に変換する関数のペア」です（GADT 構築子内の型変数 `x` は ch02 で見た存在量化です——構築時に具体型を選べますが、外からは不透明です）。`fmap` を実際に `f` に適用するのではなく、変換関数を蓄積しておきます。
...
存在量化された `x` について、`f x`（命令）と `x -> Free (Coyoneda f) a`（継続）のペアを持つ——前節で動機として描いた「命令と継続の分離」がここで正確に現れています。
...
ここまで「継続」は姿を変えながら一貫して登場してきました——ch02 の CPS では「プログラム全体の残り」として、ch03 以降のインタプリタでは「命令を処理した後、値を渡して再開する相手」として。しかしこの2つの「継続」には違いがあります。インタプリタの継続はループという**境界**の中に閉じており、プログラム全体の残りではありません。この違いに名前をつけましょう。
