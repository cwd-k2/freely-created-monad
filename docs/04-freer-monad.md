...
Free モナドの `>>=` は `fmap` で命令の内部に潜り込みます。この構造が Functor 制約を要求する原因でした。もし命令と継続を分離して保持できれば、`fmap` は不要になるはずです。この章では、その直観を **Coyoneda** で厳密に導き、Functor 制約を取り除いた **Freer モナド**を得ます。

さらに、Freer のステップ実行を整理すると、そこに**限定継続**（shift/reset）の構造が浮かび上がります。「値を外に渡して中断し、外側から値を供給されて再開する」というプロトコルは Generator の `yield`/`next` と同じであり、次章での TypeScript 実装への道を開きます。
...
存在量化された `x` について、`f x`（命令）と `x -> Free (Coyoneda f) a`（継続）のペアを持ちます。前節で動機として描いた「命令と継続の分離」がここで正確に現れています。
...
`Bind fx k >>= f` は、命令 `fx` には手を触れず、継続 `k` の後ろに `f` を合成するだけです。ch02 の `bindCont m f = \k -> m (\a -> f a k)` で見た「計算を鎖のように繋げる」構造が、命令と継続が型レベルで分離されたことで最も直接的に表現されています。`fmap` は一切使いません。
...
ここまでで Freer モナドの定義を得ました。命令と継続が型レベルで分離され、Functor 制約が消えています。以降では、この分離がインタプリタの構造にどう反映されるかを見ていきます。

## ステップ実行の整理

前章で Free モナドのインタプリタがステップ実行のパターンであることを認識しました。Freer では命令と継続が分離されたことで、このパターンをより明確に表現できます。

`Step` 型を導入します。`Step` には2つの役割があります。(1) 命令と限定継続の構造をインタプリタが読み取るための公開インターフェースとなること、(2) TypeScript の `IteratorResult` と型レベルで対応し、次章の Generator 実装への橋渡しとなること。
...
ここまで「継続」は姿を変えながら一貫して登場してきました。ch02 の CPS では「プログラム全体の残り」として、ch03 以降のインタプリタでは「命令を処理した後、値を渡して再開する相手」として。だがこの2つの「継続」には違いがあります。インタプリタの継続はループという**境界**の中に閉じており、プログラム全体の残りではありません。この違いを明確にすることで、Generator の `yield`/`next` が同じプロトコルであることが見え、次章の TypeScript 実装への橋渡しになります。
...
`Bind` の第2引数 `(\name -> ...)` が限定継続です。「`Ask` の結果 `name` を受け取って、残りの計算全体を続ける」関数であり、インタプリタ（= `reset`）の境界まで、すなわちプログラムの末尾の `Pure` までを捕捉しています。インタプリタが `k` を呼ぶと次の `Bind`（次の命令）が返ってくるため、一命令ずつステップ実行できますが、`k` 自体が保持しているのは「次の命令まで」ではなく残りの計算全体です。
